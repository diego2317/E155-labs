...
float readTemperature(void) {
    ...
    msb = spiSendReceive(0b01000100); // read msb
    spiSendReceive(0x01); // tell sensor we want to read LSB register
    lsb = spiSendReceive(0b01000101); // read lsb

    digitalWrite(SPI_CS, PIO_LOW); // CS pin low

    int16_t raw = (int16_t) ((msb << 8) | lsb);
    raw >>= 4; // sign-extend 12 bit value to 16 bits
    float temp = raw * 0.0625f; // 1 lsb = 0.0625 C
}